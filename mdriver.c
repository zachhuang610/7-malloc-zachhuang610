        ...
        switch (trace->ops[i].type) {
            ...
            case REALLOC: /* mm_realloc */
                ...
                if ((newp = mm_realloc(oldp, size)) == NULL && size) {
                    ...
                    // if the size is 0, do not proceed further since we already checked
                    // if the return value should be NULL
                    break;
                }
        ...
        switch (trace->ops[i].type) {
            case ALLOC: /* mm_alloc */
                ...
                if ((p = mm_malloc(size)) == NULL && size) {
                    ...
                    // since we already checked that the return value should be 0,
...
static void printresultsgradescope(int n, stats_t *stats) {
    ...
    for (i = 0; i < n; i++){
        if (stats[i].valid) {
            if (fprintf(fh, "%d,%s,%d,%f,%s\n", i, stats[i].trace_name, 1, stats[i].util * 100.0, stats[i].error_msg) < 0) {
                perror("failed writing to gradescope file");
                return;
            }
            util += stats[i].util;
        } else {
            if (fprintf(fh, "%d,%s,%d,%s,%s\n", i, stats[i].trace_name, 0, "-", stats[i].error_msg) < 0) {
                perror("failed writing to gradescope file");
                return;
            }
        }
...
void malloc_error(int tracenum, int opnum, char *msg) {
    ...
    if (mm_stats) {
        snprintf(mm_stats[tracenum].error_msg, sizeof(mm_stats[tracenum].error_msg),
